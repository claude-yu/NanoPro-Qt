...
class TraceParams:
    ...
    noise: int = 4                 # 杂色：去除 < noise 像素面积的斑点（filter_speckle）
    ...
    group: bool = True             # 产物所有 path 包进一个 <g> → 导入后是一个可整体移动的组（对齐 AI 扩展后的编组）
...
def _trace_diy(rgb: np.ndarray, params: TraceParams) -> tuple[str, int, dict]:
    ...
    形态学开运算只在 noise 较大(≥8)时才做，默认(noise=4)不腐蚀，1–2px 细线得以保留。
