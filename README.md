...
### 🧬 IHC / 组织化学定量 —— 对齐 Fiji，覆盖纤维化

免疫组化与组织化学染色定量，**颜色解卷积复刻 Fiji Colour Deconvolution 并校准到逐位一致**（256×256 全色域测试图三通道对 Fiji 输出 **0 像素误差**）。插件菜单「🧬 IHC / 组织化学定量」。

- **DAB 免疫组化**：颜色解卷积分离苏木素/DAB → **IHC Profiler 口径**分级（High/Positive/Low/Negative）+ 0-4 分 + DAB 阳性面积% + H-score
- **纤维化/胶原定量**：Masson 选 **Aniline blue（蓝色胶原）**通道 → **Otsu 自动阈值 + 近白背景排除** → 胶原面积%；H&E 任选通道
- **天狼星红**：无解卷积向量 → **HSV 红色胶原面积法**（亮场标准口径）。真实肾纤维化数据实测：**纤维化模型 9.3% vs sham 对照 0.3%（31×）**，正确捕获纤维化
- 拖框选多区域（无框=全图）、显示可切各解卷积通道/红色掩膜、导出 CSV；常数全部从本机 Fiji jar 反编译核对（见 `docs/ihc_alignment.md`）
...
**IHC / 组织化学定量** 颜色解卷积复刻 Fiji Colour Deconvolution（逐位一致）；DAB 免疫组化 IHC Profiler 分级/0-4 分/H-score；纤维化胶原定量（Masson Aniline blue + Otsu + 背景排除 / 天狼星红 HSV 红色面积）；按区域测量；CSV 导出
...
src/
  ...
  ihc_quant.py        IHC 定量内核（复刻 Fiji Colour Deconvolution + IHC Profiler + Otsu/天狼星红，纯 NumPy）
  ihc_analyzer.py     IHC / 组织化学定量面板（DAB 分级 / 纤维化胶原% / 天狼星红红色面积）
